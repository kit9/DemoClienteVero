...
class account_invoice(models.Model):
    ...
    def report_sunat(self):
        ...
        data['cliente_tipo_de_documento'] = int(
            self.partner_id.catalog_06_id.code) if self.partner_id.catalog_06_id else ''
        data['cliente_numero_de_documento'] = int(self.partner_id.vat) if self.partner_id else ''
        data['cliente_denominacion'] = int(self.partner_id.registration_name) if self.partner_id else ''
        data['cliente_denominacion'] = int(self.partner_id.registration_name) if self.partner_id else ''
        data['cliente_direccion'] = int(self.partner_id.registration_name) if self.partner_id else ''
        _logger.info(data)
        _logger.info(json.dumps(data, indent=3))
