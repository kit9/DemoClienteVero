...
class detracciones(models.Model):
    _name = 'sunat.type_currency'
    _description = "Tipo de Cambio Sunat"

    # campo = fields.Boolean()

    def sunat_type_currency(self):
        cambio = self.get_type_currency()
        # _logger.info("cambio -> " + str(cambio))
        moneda = self.env['res.currency'].search([('name', 'like', 'PEN')], limit=1)
        registro = self.env['res.currency.rate'].search([('name', 'like', str(datetime.now().date())),
                                                         ('currency_id', 'like', moneda.id)
                                                         ], limit=1)
        if registro:
            # _logger.info("Registro existe")
            # _logger.info(registro.name)
            registro.write({
                'rate': cambio or 0.0
            })
            # _logger.info("Registro Editado Exitozamente")
        else:
            # _logger.info("Registro no existe")
            registro_nuevo = {
                'name': str(datetime.now().date()) or False,
                'rate': cambio or 0.0,
                'currency_id': moneda and moneda.id or False
            }
            # _logger.info("Creando Registro")
            registro = self.env['res.currency.rate'].create(registro_nuevo)
            # _logger.info("Creado?")
